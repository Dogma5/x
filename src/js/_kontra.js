...
const kontra = {
   ...
   */
	init( canvas ) {
		...
		const canvasEl = this.canvas = document.getElementById( canvas )
                                 || canvas
...
( function () {
   ...
   * @param {number} [y=0] - Y coordinate.
   */
	kontra.vector = ( x, y ) => new Vector( x, y );
   ...
   * @param {function} [properties.render] - Function to use to render the sprite.
   */
	kontra.sprite = properties => ( new Sprite() ).init( properties );
...
( function () {
   ...
   */
	function loadAudio( originalUrl, url, undefined ) {
		return new Promise( ( ( resolve, reject ) => {
			...
			originalUrl = [].concat( originalUrl ).reduce( ( a, source ) => ( canUse[ getExtension( source ) ] ? source : a ), undefined );
   ...
   */
     ...
     */
		load() {
			const promises = [];
			let url; let extension; let asset; let i; let
				promise;
...
( function () {
   ...
   */
	kontra.pool = function ( properties ) {
       ...
       */
			render() {
				const index = Math.max( this.objects.length - inUse, 0 );
...
( function () {
	...
	const thisFrameRenderOrder = [];
	const lastFrameRenderOrder = [];
   ...
   */
	function circleRectCollision( object ) {
		const dx = pointer.x - Math.max( object.x, Math.min( pointer.x, object.x + object.width ) );
		const dy = pointer.y - Math.max( object.y, Math.min( pointer.y, object.y + object.height ) );
   ...
   */
	function getCurrentObject() {
		...
		const frameOrder = ( lastFrameRenderOrder.length ? lastFrameRenderOrder : thisFrameRenderOrder );
		const length = frameOrder.length - 1;
		let object; let
			collides;
     ...
     */
		untrack( objects, undefined ) {
			[].concat( objects ).map( ( object ) => {
				...
				const index = trackedObjects.indexOf( object );
				if( index !== -1 ) {
...
( function () {
   ...
   */
	kontra.quadtree = function ( properties ) {
			...
			get( object ) {
				...
				while( this.subnodes.length && this._b ) {
					...
					for( i = 0; i < indices.length; i++ ) {
						objects.push( ...this.subnodes[ indices[ i ] ].get( object ) );
					}
       ...
       */
			add() {
				let i; let j; let object; let obj; let indices; let
					index;
       ...
       */
			_g( object ) {
				...
				const intersectsTopQuadrants = object.y < horizontalMidpoint && object.y + object.height >= this.bounds.y;
				const intersectsBottomQuadrants = object.y + object.height >= horizontalMidpoint && object.y < this.bounds.y + this.bounds.height;
   ...
   */
     ...
     */
		createAnimations( animations ) {
			let animation; let frames; let frameRate; let sequence; let
				name;
   ...
   */
     ...
     */
		function preRenderImage() {
			let tile; let tileset; let image; let x; let y; let sx; let sy; let tileOffset; let
				w;
       ...
       */
			addTilesets: function addTilesets( tilesets ) {
				[].concat( tilesets ).map( ( tileset ) => {
					const tilesetImage = tileset.image;
					let image; let firstGrid; let numTiles; let lastTileset; let
						tiles;
       ...
       */
			addLayers: function addLayers( layers ) {
				[].concat( layers ).map( ( layer ) => {
					...
					let data; let r; let row; let c; let prop; let
						value;
       ...
       */
			renderLayer: function renderLayer( name ) {
				...
				let count = 0;
				let x; let y; let tile; let tileset; let image; let tileOffset; let w; let sx; let
					sy;
